                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    <thead>
                                     ...
                                     <tbody>
                                        <?php
                                           $products = mysqli_query($con, "SELECT * FROM Products ORDER BY product_date DESC");
                                           while ($product = mysqli_fetch_array($products)) { 
                                               
                                                echo "<tr class='odd gradeX'>";
                                                echo  "<td>". $product['product_name']. "</td>";
                                                 echo  "<td>". $product['product_code']. "</td>";
                                                echo  "<td>". get_category_name($product['product_category']). "</td>";
                                                echo  "<td>". get_sub_category_name($product['product_subcategory']). "</td>";
                                                echo  "<td><img style='width:200px; height:200px;' src='". $product['product_image']. "' /></td>";
                                                echo  "<td class='center'>". $product['product_date']. "</td>";
                                                echo  
                                                "<td>
                                                    <form  action='all_products.php' method='POST'>
                                                        <button type='submit' value='". $product['product_id']."' name='remove_product' class='btn btn-danger'>Delete</button>
                                                    </form>
                                                </td>";
                                                echo  
                                                "<td>
                                                    <a href='edit_product.php?id=" . $product['product_id']. "' class='btn btn-info'>Edit</a>
                                                </td>";
                                                echo  "</tr>";
                                            }
                                       ...
                                       <?php 
                                            if ($row['superuser'] != 0) {
                                                if(isset($_POST['remove_product'])) {
                                                            if (mysqli_query($con, "DELETE FROM Products WHERE product_id= '".$_POST['remove_product']."'")) {
                                                ...
                                                if(isset($_POST['remove_product'])) {
                                                            echo "You have insufficint permissions to delete an product." ;
                                                        }
